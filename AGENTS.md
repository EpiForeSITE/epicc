...
The project uses Semantic Versioning (`MAJOR.MINOR.PATCH`). Two files record the version
and must agree; `tests/epicc/test_version.py` fails if they do not:

- `src/epicc/__init__.py` (`__version__`) - the runtime source of truth. Package metadata
   is not usable: the stlite build runs from mounted sources and is never installed.
- `pyproject.toml` (`[project] version`) - also stamped into the built `index.html`.

Working rules:

- There is no changelog file in the repository. Release notes are generated by GitHub
   from the pull requests merged since the previous release, so **write pull request
   titles as user-facing release notes** - they are what people read.
- Never edit the version strings by hand. Run `make bump BUMP=minor` (or `VERSION=1.2.3`).
- Treat a change as `MAJOR` when a model document, saved preset, or shared URL produced by
   an older version stops loading.
- Pushing a `v*` tag runs `.github/workflows/release.yml`, which refuses tags that do not
   match `__version__` and otherwise publishes the release.
- The app links to the release notes rather than embedding them: `app.releases_url` in
   `src/epicc/config/default.yaml`, rendered by `src/epicc/ui/about.py`.
